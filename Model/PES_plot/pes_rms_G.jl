...
function species_energy_eV(spc_name, spc_dict, T; mode="G")
...
# %%
function state_energy_eV(species_list, spc_dict, T; mode="G")
    total = 0.0
...
# %%
function pathway_profile(path, rxn_path, rxns, spc_dict, T; mode="G")
    rows = []
...
function plot_pes(rows; title="PES", color="#1f5aa6", fontsize=14, fontweight="bold", energy_mode="G")
    ...
    ylabel(energy_mode == "G" ? "Relative Gibbs Energy (eV)" : "Relative Enthalpy (eV)", fontsize=axis_label_size, fontweight=fontweight, color="black")
...
# %%
plot_pes(rows_A, title="Path A: Formate Route", color="#1d4ed8", fontweight="bold", fontsize=14)
...
function plot_pes_fullpath(rows; title="Energy Diagram", color="#1d4ed8", fontsize=16, fontweight="black", energy_mode="G")
...
# %%
plot_pes(rows_B, title="Path B: Formate Route", color="red")
...
# %%
plot_pes(rows_C, title="Path C: CO branch", color="purple", fontsize=14, fontweight="bold")
...
# %%
plot_pes(rows_D, title="Path D: C-C coupling", color="green")
...
# %%
plot_pes(rows_E, title="Path E: C-C Hydrogenation", color="darkorange")
